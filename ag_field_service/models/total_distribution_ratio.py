...
class TotalDistributionRatio(models.Model):
    ...
    def _compute_tdr(self):
        for rec in self:
            if rec.flat_type_id:
                rec.total_resident_count = self.env['res.partner'].sudo().search_count([('contact_type', '=', 'Resident'), ('flat_type', '=', rec.flat_type_id.id), ('appartment_status', '!=', 'Consficated')])
                rec.total = rec.total_resident_count * rec.factor
            else:
                rec.total_resident_count = 0
                rec.total = 0

    @api.constrains('flat_type_id')
    def _check_tdr_type(self):
        for rec in self:
            tdr_id = self.search([('id', '!=', rec.id), ('flat_type_id', '=', rec.flat_type_id.id)])
            if tdr_id:
                raise ValidationError("TDR must be unique for each flat type!")