    ...
    def action_create_account_move(self):
        ...
        if approval_req_rec.has_amount_lines:
            for item in approval_req_rec.amounts_ids:
                item_amount = approval_req_rec.currency_id._convert(item.amount, self.env.company.currency_id, self.env.company, fields.Date.context_today(item))
                line_ids.append((0, 0, {'name': item.name, 'account_id':item.account_id.id, 'debit':item_amount, 'currency_id':approval_req_rec.currency_id.id}))
        else:
            item_amount = approval_req_rec.currency_id._convert(approval_req_rec.amount, self.env.company.currency_id, self.env.company, fields.Date.today())
            line_ids.append((0, 0, {'name':approval_req_rec.name, 'account_id':approval_req_rec.account_id.id, 'debit':item_amount, 'currency_id':approval_req_rec.currency_id.id}))
        
#         move_id = self.env['account.move'].create({})

        for line in self.payment_line:
            item_amount = approval_req_rec.currency_id._convert(line.amount, self.env.company.currency_id, self.env.company, fields.Date.context_today(line))
            line_ids.append((0, 0, {'name':line.name, 'account_id':line.account_id.id, 'credit':item_amount, 'currency_id':approval_req_rec.currency_id.id}))
        ...
        # for line in move_id.line_ids:
            # line._onchange_ag_debit()
            # line._onchange_ag_credit()
        ...
        if approval_req_rec.approval_type == 'direct_procurement':
            store_officer_ids = approval_req_rec.company_id.store_user_ids
...
class PaymentAmountLineApproval(models.TransientModel):
    ...
    # account_id = fields.Many2one('account.account', string='Credit Account', domain="['|',('account_type', '=', 'asset_current'), ('name', 'ilike','Outstanding Payments')]", required=True)
    amount = fields.Float(string="Amount", required=True)
    # Added new domain Petty Cash
    account_id = fields.Many2one(
        'account.account',
        string='Credit Account',
        domain="['|', '|',('account_type', '=', 'asset_current'), ('name', 'ilike', 'Outstanding Payments'), ('name', 'ilike', 'Petty Cash')]",
        required=True
    )
