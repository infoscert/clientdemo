...
class ApprovalRequest(models.Model):
    ...
    is_account_editable = fields.Boolean("Is Account Editable?", compute="_compute_is_account_editable")
    ...
    region_location_id = fields.Many2one("region.location", string="Location")
    ...
    overall_total_amount = fields.Monetary("Total Amount", compute="_compute_overall_total_amount")
    show_create_rfq = fields.Boolean("Show Create RFQ", compute="_compute_show_create_rfq")


    def _compute_is_account_editable(self):
        current_user = self.env.user
        for rec in self:
            rec.is_account_editable = False
            if current_user.has_group('base.group_erp_manager'):
                rec.is_account_editable = True
            if rec.approval_type == 'payment' and current_user.id in rec.category_id.approver_ids.mapped('user_id').ids:
                rec.is_account_editable = True
    ...
    @api.depends('product_line_ids', 'product_line_ids.amount', 'nonsku_product_line_ids', 'nonsku_product_line_ids.amount', 'amounts_ids')
    def _compute_total_amount(self):
        # overall_total_amount = 0
        for rec in self:
            if rec.approval_type != 'payment':
                ...
                # overall_total_amount = rec.total_amount + rec.nonsku_total_amount
            if rec.approval_type == 'payment':
                rec.amount = sum(rec.amounts_ids.mapped('amount'))
            #     overall_total_amount = rec.amount
            # rec.overall_total_amount = overall_total_amount

    def _compute_overall_total_amount(self):
        for rec in self:
            if rec.approval_type == 'payment':
                rec.overall_total_amount = rec.amount
            else:
    ...
    #             body_dynamic_html += '<p> Approver: %s </p>' % self.env.user.name
            ...
            if self.approval_type == 'payment':
                for rep in self.region_location_id.account_rep_id:
...
#         return res
    ...
    @api.onchange('category_id', 'request_owner_id', 'approval_type', 'region_location_id')
    def _onchange_category_id(self):
        if self.category_id.approval_type == 'payment' and self.region_location_id:
            self.acc_dep_rep = [(6,0, self.region_location_id.account_rep_id.ids)] #self.category_id.to_process_payment_user_id
...
#         return super().action_confirm()
    ...
    def action_create_purchase_orders(self):
        ...
        if self.product_line_ids:
            ...
            for line in self.product_line_ids:
                ...
                new_purchase_order.location = self.region_location_id.id
    ...
    def action_release_products(self):
        warehouse_id = self.env['stock.warehouse'].sudo().search([], limit=1)
